...
export default JobBiddingDetails = props => {
  ...
  const [Biddatadetail, setBiddatadetail] = useState(false);
    ...
    axios
      ...
      .then(response => {
        ...
        if (response?.data?.success === true) {
          setIsLoading(false);
          console.log('getContractor....', response?.data?.data);
          setgetContractorData(response?.data);
          setBiddatadetail(true);
        } else {
          console.error('getContractor_error:', response?.data?.error);
          setIsLoading(false);
        }
  ...
  const renderItem = ({item}) => (
    <>
      {getContractorData?.data[0]?.result ? null : (
        <>
  ...
  return (
    <View>
      {!Biddatadetail || getContractorData?.data >= 0 ? (
        <>
