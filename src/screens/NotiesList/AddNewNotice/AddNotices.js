...
import { View, Text, TextInput, TouchableOpacity, SafeAreaView, KeyboardAvoidingView, Alert, ScrollView, FlatList, Image } from 'react-native'
  ...
  const [NoticeAllData, setNoticeAllData] = useState(null);
    ...
    const handle_notification_type = async () => {
        ...
        try {
            ...
            console.error('API call setNotification_type_Data failed:', error.message);
            alert(error.message); // Set error state for UI feedback
        } finally {
            setIsLoading(false);
        }
        ...
        <SafeAreaView style={AddNewNoticeStyle.MainContainer}>
            ...
            {IsMap ? (
                ...
                <KeyboardAvoidingView
                    ...
                    <ScrollView style={AddNewNoticeStyle.mainview} showsVerticalScrollIndicator={false}>
                            ...
                            <View style={AddNewNoticeStyle.jobDetailsView}>
                                ...
                                <View style={{ flex: 1, flexDirection: 'row' }}>
                                    <TouchableOpacity
                                        ...
                                        <Text style={{ flex: 1,alignSelf:'center',fontSize:15,color:displaySelectedValues || NoticeAllData?.guests? _COLORS.Kodie_BlackColor: _COLORS.Kodie_LightGrayColor }}>{displaySelectedValues ? displaySelectedValues : NoticeAllData?.guests ? NoticeAllData?.guests:"Add guests"}</Text>
                                        <Feather
                                            name="user-plus"
                                            size={30}
                                            color={_COLORS.Kodie_GrayColor}
